...
class IMDBDataset(torch.utils.data.Dataset):
    def __init__(self, data_config : omegaconf.DictConfig, split : Literal['train', 'valid', 'test']):
        ...
        # 데이터를 8:1:1 비율로 나누기
        train_test = data_total.train_test_split(test_size=0.1)  # 전체 데이터를 9:1로 나누기
        train_valid = train_test['train']  # 90% 훈련 데이터
        test = train_test['test']  # 10% 테스트 데이터

        # train를 다시 8:1 씩 나누기
        train_valid = train_valid.train_test_split(test_size=1/9)
        ...
        self.data = self.data.map(lambda example: self.tokenizer(example['text'], truncation=True, padding='max_length', max_length=self.max_len))
    ...
    def __getitem__(self, idx) -> Tuple[dict, int]:
        ...
        inputs['label'] = self.data['label'][idx]
        if 'token_type_ids' in self.data:
            inputs["token_type_ids"] = self.data['token_type_ids'][idx]
        return inputs
